...
describe('GET /busynessRating', () => {
  ...
  it('should return 200 and prediction if model is retrieved and no error occurs', async () => {
    ...
    ml.getBusynessPredictions = jest.fn().mockResolvedValueOnce(dummyPredictions);

    const response = await request(app).get(`?datetime=${testDateTime}`);
    
    expect(response.status).toBe(200);
    expect(response.body).toEqual(dummyPredictions);
    expect(ml.getBusynessPredictions).toHaveBeenCalledWith(testDateTime);
  });

  it('should return 500 if database error occurs', async () => {
    ml.getBusynessPredictions = jest.fn().mockImplementation(() => {
